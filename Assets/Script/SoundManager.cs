...
public class SoundManager : MonoBehaviour {
	...
	public List<AudioClipData> m_bgmAudioClipData = new List<AudioClipData> ();
	...
	void initialize (){
		...
		foreach (AudioClipData data in m_bgmAudioClipData) {
			m_tAudioClipDataList [(int)AUDIO_TYPE.BGM].Add (data);
		}
		return;
	}
...
//		InitSoundSettingData ();
	...
	public Queue<string > m_strLoadFilenameQueue = new Queue<string>();
	...
	public int AddLoadFile( string _strFilename ){
		m_strLoadFilenameQueue.Enqueue (_strFilename);
		return m_strLoadFilenameQueue.Count;
	}
	public bool IsIdle(){
		if (m_eStep == STEP.IDLE && m_strLoadFilenameQueue.Count == 0) {
			return true;
		}
		return false;
	}
