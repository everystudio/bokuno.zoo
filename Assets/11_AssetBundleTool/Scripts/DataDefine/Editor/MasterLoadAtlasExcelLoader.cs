...
public class MasterLoadAtlasExcelLoader : EditorWindow
{
	...
	{
		ReadExcel (typeof( MasterLoadAtlas));
	}

	private static List<MasterLoadAtlas.Data> ReadExcel (Type _type)
	{
		string loadPath = SystemSetting.GetExcelSeetPath () + _type.Name + "Sheet.xls";
		List<MasterLoadAtlas.Data> dataList = new List<MasterLoadAtlas.Data> ();
		...
		using (FileStream stream = File.Open (loadPath, FileMode.Open, FileAccess.Read)) {
			...
			for (int i = 1; i < sheet.LastRowNum; i++) {
				...
				MasterLoadAtlas.Data tmpData = new MasterLoadAtlas.Data ();
				...
				tmpData.replace_prefab = row.GetCell (iColumn++).StringCellValue;
		...
		MasterLoadAtlasCSVLoader.UpdateCSVFile(dataList);
	...
	/// データの書き込み.
	/// </summary>
	public static void WriteExcelData ( List<MasterLoadAtlas.Data> listData, Type _dataType, Type _dataTypeSrc)
	{
	...
	public static void MakeCell (ISheet _sheet1, Type _dataType, int _iRow, List<MasterLoadAtlas.Data> listData)
	{
		//Type makeDataType = typeof(List<MasterLoadAtlas.Data>);
		...
		Type tmpType = typeof(MasterLoadAtlas.Data);
		MasterLoadAtlas.Data tmpData = listData[curDataRow];
