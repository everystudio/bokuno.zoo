...
public class MasterLoadPrefabExcelLoader : EditorWindow
{
	...
	{
		ReadExcel (typeof( MasterLoadPrefab));
	}

	private static List<MasterLoadPrefab.Data> ReadExcel (Type _type)
	{
		string loadPath = SystemSetting.GetExcelSeetPath () + _type.Name + "Sheet.xls";
		List<MasterLoadPrefab.Data> dataList = new List<MasterLoadPrefab.Data> ();
		...
		using (FileStream stream = File.Open (loadPath, FileMode.Open, FileAccess.Read)) {
			...
			for (int i = 1; i < sheet.LastRowNum; i++) {
				...
				MasterLoadPrefab.Data tmpData = new MasterLoadPrefab.Data ();
		...
		MasterLoadPrefabCSVLoader.UpdateCSVFile(dataList);
	...
	/// データの書き込み.
	/// </summary>
	public static void WriteExcelData ( List<MasterLoadPrefab.Data> listData, Type _dataType, Type _dataTypeSrc)
	{
	...
	public static void MakeCell (ISheet _sheet1, Type _dataType, int _iRow, List<MasterLoadPrefab.Data> listData)
	{
		//Type makeDataType = typeof(List<MasterLoadPrefab.Data>);
		...
		Type tmpType = typeof(MasterLoadPrefab.Data);
		MasterLoadPrefab.Data tmpData = listData[curDataRow];
