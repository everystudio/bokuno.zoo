...
public class MasterCommonShopAtach : MonoBehaviour
{
	...
	public static void AtachFolder ()
	{
		...
		CreateComponents (typeof(MasterCommonShop));
	}
	...
	public static void CreateComponents (Type _type)
	{
		...
		GameObject gameObject = EditorUtility.CreateGameObjectWithHideFlags (
			...
			typeof(MasterCommonShopDataHolder)
		);
		...
		MasterCommonShop tmp = new MasterCommonShop();

		foreach (UnityEngine.Object asset in assets) {
			if (asset is MasterCommonShop) {
				tmp = (MasterCommonShop)asset;
			}
		}

		MasterCommonShopDataHolder holder = gameObject.GetComponent<MasterCommonShopDataHolder> ();
