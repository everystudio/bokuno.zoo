...
public class MasterLoginBonusAtach : MonoBehaviour
{
	...
	public static void AtachFolder ()
	{
		...
		CreateComponents (typeof(MasterLoginBonus));
	}
	...
	public static void CreateComponents (Type _type)
	{
		...
		GameObject gameObject = EditorUtility.CreateGameObjectWithHideFlags (
			...
			typeof(MasterLoginBonusDataHolder)
		);
		...
		MasterLoginBonus tmp = new MasterLoginBonus();

		foreach (UnityEngine.Object asset in assets) {
			if (asset is MasterLoginBonus) {
				tmp = (MasterLoginBonus)asset;
			}
		}

		MasterLoginBonusDataHolder holder = gameObject.GetComponent<MasterLoginBonusDataHolder> ();
